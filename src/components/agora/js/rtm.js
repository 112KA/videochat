...
export default class RTMClient extends EventDispatcher {
  constructor() {
    super()
    this.channels = {}
    this._logined = false
  ...
  subscribeChannelEvents(channelName) {
    ...
    channelEvents.forEach((eventName) => {
      this.channels[channelName].channel.on(eventName, (...args) => {
        console.log('emit ', eventName, args)
        this.dispatch(eventName, { channelName, args: args })
      })
    })
  }
  ...
  async getChannelMemberCount(channel) {
    return await this.client.getChannelMemberCount([channel])
  }
